...
export default function PengaturanClient({ profile, settings }: Props) {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-7xl mx-auto space-y-6 sm:space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Pengaturan Sistem</h1>
        <p className="text-muted-foreground mt-1 text-xs sm:text-sm">Kelola preferensi akun dan konfigurasi inventaris utama Anda.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
        ...
        <div className="lg:col-span-2 space-y-6 sm:space-y-8">
          ...
          <form action={profileAction}>
            <div className="bg-[#fcfbf7] p-4 sm:p-8 rounded-2xl border border-[#e5dcd1] shadow-sm">
              <div className="flex items-center gap-3 mb-6 sm:mb-8 pb-4 border-b border-[#e5dcd1]">
                <User className="text-[#8a6c5f] w-5 h-5" />
                <h2 className="text-base sm:text-lg font-semibold text-foreground">Profil Admin</h2>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-6 sm:gap-10 items-center sm:items-start">
                <div className="flex flex-col items-center">
                  <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-full overflow-hidden border-4 border-white shadow-md bg-[#f0e8df] flex items-center justify-center">
                    <User className="w-12 h-12 sm:w-16 sm:h-16 text-[#8a6c5f]" />
                  </div>
                </div>
                <div className="flex-1 space-y-4 sm:space-y-6 w-full">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    <div className="space-y-2 sm:space-y-3">
                      ...
                      <Input name="name" defaultValue={profile?.name || ""} className="bg-[#fcfbf7] border-[#e5dcd1] h-10 sm:h-12" />
                    </div>
                    <div className="space-y-2 sm:space-y-3">
                      <Label className="text-sm font-medium">Peran</Label>
                      <Input defaultValue="Administrator" disabled className="bg-[#f0e8df]/50 border-[#e5dcd1] text-muted-foreground h-10 sm:h-12" />
                    </div>
                  </div>
                  <div className="space-y-2 sm:space-y-3">
                    <Label className="text-sm font-medium">Email</Label>
                    <Input name="email" defaultValue={profile?.email || ""} className="bg-[#fcfbf7] border-[#e5dcd1] h-10 sm:h-12" />
                  </div>
                  <div className="flex justify-end pt-2 sm:pt-4">
                    <Button type="submit" disabled={profilePending} className="w-full sm:w-auto bg-[#8a6c5f] hover:bg-[#6b5247] text-white h-10 sm:h-12 px-8 rounded-xl font-medium">
                      Simpan Profil
                    </Button>
                  </div>
                </div>
          ...
          <form action={pwAction}>
            <div className="bg-[#fcfbf7] p-4 sm:p-8 rounded-2xl border border-[#e5dcd1] shadow-sm">
              <div className="flex items-center gap-3 mb-6 sm:mb-8 pb-4 border-b border-[#e5dcd1]">
                <User className="text-[#8a6c5f] w-5 h-5" />
                <h2 className="text-base sm:text-lg font-semibold text-foreground">Ubah Password</h2>
              </div>
              {pwState?.message && (
              ...
              <div className="space-y-4 sm:space-y-6">
                <div className="space-y-2 sm:space-y-3">
                  ...
                  <Input name="currentPassword" type="password" className="bg-[#fcfbf7] border-[#e5dcd1] h-10 sm:h-12" />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  <div className="space-y-2 sm:space-y-3">
                    <Label>Password Baru</Label>
                    <Input name="newPassword" type="password" className="bg-[#fcfbf7] border-[#e5dcd1] h-10 sm:h-12" />
                  </div>
                  <div className="space-y-2 sm:space-y-3">
                    <Label>Konfirmasi Password Baru</Label>
                    <Input name="confirmPassword" type="password" className="bg-[#fcfbf7] border-[#e5dcd1] h-10 sm:h-12" />
                  </div>
                </div>
                <div className="flex justify-end pt-2 sm:pt-4">
                  <Button type="submit" disabled={pwPending} className="w-full sm:w-auto bg-[#8a6c5f] hover:bg-[#6b5247] text-white h-10 sm:h-12 px-8 rounded-xl font-medium">
                    Ubah Password
                  </Button>
                </div>
              </div>
          ...
          <div className="bg-[#fcfbf7] p-8 rounded-2xl border border-[#e5dcd1] shadow-sm">
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-4">
                {[
                  { icon: Tag, label: "Kategori Produk", href: null },
                  { icon: Truck, label: "Daftar Supplier", href: null },
                  { icon: Building, label: "Metode Pembayaran", href: "/pengaturan/metode-pembayaran" },
                  { icon: ArrowLeftRight, label: "Kategori Cashflow", href: null },
                  ...
                  const content = (
                    <>
                      <div className="flex items-center gap-4">
                        ...
                        <span className="text-sm font-semibold text-[#2d201c]">{label}</span>
                      </div>
                      <ChevronRight className="w-5 h-5 text-muted-foreground" />
                    </>
                  );

                  const className = "w-full flex items-center justify-between p-4 rounded-xl border border-[#e5dcd1] bg-[#fdfbf7] hover:bg-[#f0e8df] transition-colors group text-left cursor-pointer";
