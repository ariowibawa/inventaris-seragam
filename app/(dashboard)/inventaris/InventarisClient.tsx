...
type InventoryProduct = {
  ...
  stock: number; minimumStock: number; isActive: boolean;
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">Kelola stok seragam dan riwayat pergerakan barang.</p>
        </div>
        <Dialog open={adjustOpen} onOpenChange={setAdjustOpen}>
          <DialogTrigger render={<Button className="bg-[#8a6c5f] hover:bg-[#6b5247] text-white" onClick={() => { setSelectedProductId(""); setAdjustType("in"); setAdjustOpen(true); }} />}>
          ...
          <DialogContent className="sm:max-w-[500px] p-0 border-[#e5dcd1] bg-[#fdfbf7] shadow-lg overflow-hidden" showCloseButton={false}>
            <div className="flex justify-between items-center p-6 border-b border-[#e5dcd1]">
              <DialogTitle className="text-xl font-semibold text-foreground">Penyesuaian Stok</DialogTitle>
              ...
              <div className="p-6 space-y-4">
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="flex h-8 w-full items-center rounded-lg border border-[#e5dcd1] bg-white px-2.5 py-2 text-sm outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50"
                  >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#fdfbf7] p-6 rounded-2xl border border-[#e5dcd1] shadow-sm flex flex-col justify-between">
          <div className="w-10 h-10 rounded-lg bg-[#f0e8df] flex items-center justify-center mb-4"><Shirt className="h-5 w-5 text-[#8a6c5f]" /></div>
          <div><p className="text-sm text-muted-foreground font-medium">Total Produk</p><p className="text-3xl font-bold text-foreground mt-1">{fmtNum(summary.totalProducts)}</p></div>
        </div>
        <div className="bg-[#fdfbf7] p-6 rounded-2xl border border-[#e5dcd1] shadow-sm flex flex-col justify-between">
          <div className="w-10 h-10 rounded-lg bg-[#f0e8df] flex items-center justify-center mb-4"><Package className="h-5 w-5 text-[#8a6c5f]" /></div>
          <div><p className="text-sm text-muted-foreground font-medium">Total Stok</p><p className="text-3xl font-bold text-foreground mt-1">{fmtNum(summary.totalStock)}</p></div>
        </div>
        <div className="bg-[#fdfbf7] p-6 rounded-2xl border border-[#e5dcd1] shadow-sm flex flex-col justify-between">
          <div className="w-10 h-10 rounded-lg bg-orange-100 flex items-center justify-center mb-4"><AlertTriangle className="h-5 w-5 text-orange-600" /></div>
          <div><p className="text-sm text-muted-foreground font-medium">Stok Hampir Habis</p><p className="text-3xl font-bold text-orange-600 mt-1">{summary.lowStockCount}</p></div>
        </div>
        <div className="bg-[#fdfbf7] p-6 rounded-2xl border border-[#e5dcd1] shadow-sm flex flex-col justify-between">
          <div className="w-10 h-10 rounded-lg bg-red-100 flex items-center justify-center mb-4"><AlertCircle className="h-5 w-5 text-red-600" /></div>
          <div><p className="text-sm text-muted-foreground font-medium">Stok Habis</p><p className="text-3xl font-bold text-red-600 mt-1">{summary.outOfStockCount}</p></div>
        </div>
      </div>
      ...
      <div className="bg-[#fdfbf7] border border-[#e5dcd1] rounded-2xl shadow-sm overflow-hidden flex flex-col">
        <div className="p-5 border-b border-[#e5dcd1] flex flex-col sm:flex-row justify-between items-center gap-4">
          <h2 className="text-lg font-semibold text-foreground">Daftar Inventaris</h2>
          <div className="flex gap-3 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-[280px]">
              <Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />
              <Input className="pl-9 bg-white border-[#e5dcd1]" placeholder="Cari SKU atau nama produk..."
                value={searchValue} onChange={(e) => setSearchValue(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter") updateSearch(); }} />
            </div>
            <Button variant="outline" className="bg-white border-[#e5dcd1] text-foreground gap-2" onClick={updateSearch}>
      ...
      <Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
        <DialogContent className="sm:max-w-[600px] p-0 border-[#e5dcd1] bg-[#fdfbf7] shadow-lg overflow-hidden" showCloseButton={false}>
          <div className="flex justify-between items-center p-6 border-b border-[#e5dcd1]">
